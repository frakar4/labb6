...
public class CarWashView extends SimView {
	...
	public void update(Observable o, Object arg) {
		...
		if (arg instanceof StartEvent) {
			...
			updateInfo = String.format(stopTemplate, event.getTime(), "Stop", state.getAvailableFast(),
					state.getAvailableSlow(), state.getTotalIdleTime(), state.getTotalQueueTime(),
					state.getCarsInQueue(), state.getRejectedCars());
			...
			updateInfo = String.format(template, event.getTime(), "Arrive", event.getCar().getCarID(),
					state.getAvailableFast(), state.getAvailableSlow(), state.getTotalIdleTime(),
					state.getTotalQueueTime(), state.getCarsInQueue(), state.getRejectedCars());
		} else if (arg instanceof Leave) {
			Leave event = (Leave) arg;
			updateInfo = String.format(template, event.getTime(), "Leave", event.getCar().getCarID(),
					state.getAvailableFast(), state.getAvailableSlow(), state.getTotalIdleTime(),
					state.getTotalQueueTime(), state.getCarsInQueue(), state.getRejectedCars());
		} else {
	...
	public void beforeRun() {
		...
		String message = "Fast Machines: " + state.getTotalFast() + "\n" + "Slow Machines: " + state.getTotalSlow()
				...
				+ ")\n" + "Slow Distribution: (" + state.getSlowDistribution()[0] + ", "
				+ state.getSlowDistribution()[1] + ")\n" + "Exponential distribution with lambda = " + state.getLambda()
				+ "\n" + "Seed = " + state.getSeed() + "\n" + "Max Queue Size: " + state.getMaxQueueSize() + "\n"

				+ "-----------------------------------------------------------------------------------------------";
		System.out.println(message);
		System.out.printf(titleTemplate, "Time", "Event", "Id", "Fast", "Slow", "IdleTime", "QueueTime", "QueueSize",
				"Rejected");
	...
	public void afterRun() {
		...
		System.out.printf("%-25s %5.2f%n", "Mean queueing time: ",
				state.getTotalQueueTime() / (biggestId + 1 - state.getRejectedCars()));
